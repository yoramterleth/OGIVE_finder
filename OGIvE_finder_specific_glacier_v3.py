# -*- coding: utf-8 -*-
"""
Created on Thursday sep 22nd 16:01 2022
...
This script pulls the elevation and slope profiles of ArcticDEM along all the centerlines that are significant from the RGI database 
by the user.
...
#%% select a glacier #####################################################################
RGIglacier = ee.FeatureCollection('GLIMS/current').filter('glac_name=="Hubbard Glacier"')
...
#centerlines = gp.read_file('C:/Users/Yoram/OneDrive - University of Idaho\Desktop/PhD pos/OGIVES/findr/centerlines/cl_Gilkey.shp')
...
for line in cl.geometry:
    ...
    print('###### Working on new centerline in glacier ######')
    
    #%% merge linestrings in a multiline string together when necessarry 
    try: 
        test = line[0]
        outcoords = [list(i.coords) for i in line]

        # Flatten the list of sublists and use it to make a new line
        centerline = shapely.geometry.LineString([i for sublist in outcoords for i in sublist])
    ...
    # skip this cl if it is very short 
    if ee_centerline.length().getInfo() < 1000:
        print('Skipped a centerline that is '+ str(ee_centerline.length().getInfo())+ 'm long.')
        continue
    ...
    try:
        ...
        print('No data in this line, skip...')
        continue
    ...
    select_xx = xx[interp_select_array> 1 ] #np.percentile(interp_select_array,90)] # 1.5 
    select_yy = yy[interp_select_array> 1 ] #np.percentile(interp_select_array,90)] # 1.5
    select_amp = interp_select_array[interp_select_array>np.percentile(interp_select_array,90)] 
    
    print('Saving locations...')
    
    O_xx = np.append(O_xx, select_xx)
    O_yy = np.append(O_yy, select_yy)
    O_amp = np.append(O_amp, select_amp)
